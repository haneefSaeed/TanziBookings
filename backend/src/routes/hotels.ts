...
async(req: Request, res: Response)=>{
  ...
  // hotel ID tie payment to hotel
  // userId who created this booking
...
router.post("/:hotelId/bookings", verifyToken, async(req: Request, res:Response)=>{
  try {
    // check if stripe payment is succesful
    // the front end will send these information to backend to check 
    ...
    if(paymentIntent.metadata.hotelId!=req.params.hotelId || 
      ...
      // ensure hotel booking is realted to user ID !! 
      return res.status(400).json({message: "MetatData not found"});

    }

    if(paymentIntent.status != "succeeded"){
      // tell the client what went wrong
      return res.status(400).json({message: "payment intent not succeeded: "  + paymentIntent.status})
    }
