...
export type SearchParams = {
    // all string because we use params in URl it has to be string 
    destination?: string;
    checkIn? : string;
    checkOut?: string;
    adultCount? : string;
...
export const searchHotels= async(searchParams: SearchParams): Promise<HotelSearchResponse> =>{
    ...
    searchParams.facilities?.forEach(fac=>(queryParams.append("facilities", fac)));
...
export const fetchHotelDetailById = async(hotelId: string)=>{
...
:Promise<paymentIntentResponse>=>{
    ...
    fetch(`${API_BASE_URL}/api/hotels/${hotelId}/bookings/payment-intent`, {
        credentials: 'include', 
        method: "POST",
...
export const createRoomBooking = async(formData: BookingFormData)=>{
    const response = await 
    fetch(`${API_BASE_URL}/api/hotels/${formData.hotelId}/bookings`, {
        method: "POST",
        headers: {
            "Content-Type" : "application/json"
        }, 
    ...
    if(!response.ok) throw new Error("Erorr booking the room");
...
export const fetchMyBookings = async(): Promise<HotelType[]>=>{
    ...
    if(!response.ok) throw new Error("Error on fetching bookings")

    return response.json();
}


export const fetchHomePageItems = async(): Promise<HotelType[]> =>{
    const response = await fetch(API_BASE_URL + "/api/hotels");
    if(!response.ok) throw new Error("Error at fetching hotels");
    return response.json();
}